function [imDeconv, imPeak]  = gpuWindDeconv(imBS,Sigma,imBG,minIntensity,W)
% ------------------------------------------------------------------------------------
...
% WW(imB>index^2) = 0;
% W = fftshift(WW);
imDeconv = gpuArray.zeros(imW,imH,imF,'single');
imPeak = gpuArray.zeros(imW,imH,imF,'single');
for f = 1:imF
    im = imBS(:,:,f);
    G = fft2(im);
    F = W.*G;
